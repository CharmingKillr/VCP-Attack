...
def run_claude_captioning(out_file, dataloader=None, prompt=None, max_tokens=None, batch_size=None, num_samples=None):
    client = Anthropic(
    ...
    for idx, (_, image_path) in enumerate(tqdm(dataloader, desc="Processing images")):
        ...
        caption = response.content[0].text.strip().replace('\n', ' ')
        with open(out_file, 'a') as f:
            f.write(caption + '\n')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Claude Image Captioning Script")
    ...
    parser.add_argument("--output_path", default="/data/jw/mnt/mount/VCP-Attack/output_adv_captions_1000/claude-3-7-sonnet-20250219", type=str)
    ...
    run_claude_captioning(output_file, dataloader, args.query, args.max_new_tokens, args.batch_size, args.num_samples)
